...
async function genEntireInput(stateTree, txs, signatureArray, maxTxs) {
    ...
    assert(numTxs == signatureArray.length);
    ...
    for (let i = 0; i < numTxs; i++) {
        let input = await genSingleTxInput(stateTree, senderAccountIdArray[i], receiverAccountIdArray[i], amountArray[i], signatureArray[i]);
